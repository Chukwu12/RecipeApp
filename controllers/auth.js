...
exports.logout = (req, res, next) => {
  req.logout((err) => {
    ...
    // If there is no active session, just redirect safely.
    if (!req.session) {
      req.user = null;
      return res.redirect("/");
    }
    ...
    req.session.destroy((destroyErr) => {
      if (destroyErr) {
        console.log("Error : Failed to destroy the session during logout.", destroyErr);
      }
      req.user = null;
      res.redirect("/");
    });
...
exports.getSignup = (req, res) => {
  ...
  res.render("signup", {
    ...
    errors: req.flash("errors"),  // Pass the validation errors to the view
  });
};
